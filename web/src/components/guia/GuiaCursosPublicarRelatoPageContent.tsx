    ...
    <div className="mt-8 w-full">
      ...
      <section className="mt-10 rounded-3xl border border-brand-200/40 bg-brand-100/30 p-6 md:p-8">
        ...
        <form className="mt-7 grid gap-6" onSubmit={handleSubmit} onChange={handleFieldChange} onBlur={handleFieldBlur} noValidate>
          ...
          <div className="grid gap-4 sm:grid-cols-2"><label className="grid gap-2 text-sm font-bold text-neutral-500"><span className="inline-flex items-center gap-1.5">Valor investido <span className="font-normal text-neutral-400">(opcional)</span></span><Input name="investment" inputMode="numeric" value={investment} onChange={(event) => setInvestment(event.target.value ? formatCurrency(event.target.value) : '')} className="h-10 bg-neutral-100" placeholder="R$0,00" /><FieldError message={fieldErrors.investment} /></label><label className="grid gap-2 text-sm font-bold text-neutral-500"><span>Modalidade <b className="text-brand-500">*</b></span><select name="modality" required aria-invalid={Boolean(fieldErrors.modality)} className="h-10 rounded-xl border border-neutral-500/15 bg-neutral-100 px-3 py-2.5 font-normal aria-[invalid=true]:border-red-600"><option value="">Selecione</option><option>Online</option><option>Presencial</option><option>Híbrido</option></select><FieldError message={fieldErrors.modality} /></label></div>
