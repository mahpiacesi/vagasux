export type GuiaFerramentaLink = {
  title: string
  url: string
  description?: string
  previewImageUrl?: string
}

export const guiaFigmaLinks: GuiaFerramentaLink[] = [
  {
    title: 'Boas práticas do Figma',
    description: 'Boas práticas para usar a ferramenta no dia a dia.',
    url: 'https://www.figma.com/best-practices/',
  },
  {
    title: 'Fundamentos de design',
    description: 'Recursos para revisar princípios essenciais de design.',
    url: 'https://www.figma.com/resource-library/design-basics/',
  },
  {
    title: 'Dicas e truques',
    description: 'Guias rápidos para aproveitar melhor os recursos do Figma.',
    url: 'https://www.figma.com/best-practices/tips-and-tricks/',
  },
  {
    title: 'Organização de arquivos',
    description: 'Estruture times, projetos e arquivos de forma mais clara.',
    url: 'https://www.figma.com/best-practices/team-file-organization/',
  },
  {
    title: 'Notas de lançamento',
    description: 'Acompanhe as novidades e atualizações da plataforma.',
    url: 'https://www.figma.com/release-notes/',
  },
  {
    title: 'Comunidade Figma',
    description: 'Encontre arquivos, plugins e recursos criados pela comunidade.',
    url: 'https://www.figma.com/community',
  },
  {
    title: 'Figma para iniciantes',
    description: 'Curso gratuito para começar a criar interfaces no Figma.',
    url: 'https://www.cursae.com.br/play-course/figma-criando-interfaces-do-zero-ate-o-prototipo-final/introducao-ao-figma',
  },
  {
    title: 'Friends of Figma',
    description: 'Eventos e grupos locais para conectar a comunidade Figma.',
    url: 'https://friends.figma.com/',
  },
  {
    title: 'Friends of Figma São Paulo',
    description: 'Acompanhe a comunidade e os encontros de Figma em São Paulo.',
    url: 'https://friends.figma.com/sao-paulo/',
  },
  {
    title: 'Figma no X',
    description: 'Canal oficial para novidades e referências da Figma.',
    url: 'https://x.com/figma',
  },
  {
    title: 'Blog da Figma',
    description: 'Artigos sobre produto, design e o universo da Figma.',
    url: 'https://www.figma.com/blog/',
  },
  {
    title: 'Config',
    description: 'Conferência oficial da Figma sobre design e construção de produtos.',
    url: 'https://config.figma.com/',
  },
]

export const guiaFerramentasLinks: GuiaFerramentaLink[] = [
  {
    title: 'Framer',
    description: 'Crie sites responsivos e experiências interativas sem começar pelo código.',
    url: 'https://framer.com/',
  },
  {
    title: 'Lunacy',
    description: 'Editor gratuito para design de interfaces com recursos de IA.',
    url: 'https://icons8.com.br/lunacy',
    previewImageUrl:
      'https://maxst.icons8.com/vue-static/landings/lunacy-new/opengraph-lunacy.jpg',
  },
  {
    title: 'Webflow',
    description: 'Construa e publique sites visuais com CMS integrado.',
    url: 'https://webflow.com/',
  },
  {
    title: 'Sketch',
    description: 'Ferramenta de design de interfaces feita para macOS.',
    url: 'https://www.sketch.com/',
    previewImageUrl: 'https://www.sketch.com/images/metadata/pages/home.jpg',
  },
  {
    title: 'Axure',
    description: 'Crie protótipos detalhados, diagramas e especificações.',
    url: 'https://www.axure.com/',
    previewImageUrl: 'https://www.axure.com/images/2025/04/og-image.png',
  },
  {
    title: 'UXPin',
    description: 'Prototipe com componentes, lógica e estados mais realistas.',
    url: 'https://www.uxpin.com/',
    previewImageUrl:
      'https://files.uxpin.com/marketing-assets/hEANM0hTqezomJsApfaQ.webp',
  },
  {
    title: 'Marvel',
    description: 'Faça wireframes e protótipos clicáveis de forma rápida.',
    url: 'http://marvelapp.com/',
    previewImageUrl:
      'https://marvelapp.com/static/workshop-nav@2x-7ec05d5fe3d476739e3d47833e3f8956-c4bfe.jpg',
  },
  {
    title: 'Penpot',
    description: 'Ferramenta open source para design e colaboração em equipe.',
    url: 'https://penpot.app/',
  },
  {
    title: 'ProtoPie',
    description: 'Crie interações avançadas para protótipos sem código.',
    url: 'https://www.protopie.io/',
  },
  {
    title: 'Canva',
    description: 'Monte apresentações e peças visuais com modelos prontos.',
    url: 'https://www.canva.com/pt_br/',
  },
]

export const guiaQuadroDeIdeiasLinks: GuiaFerramentaLink[] = [
  { title: 'FigJam', description: 'Quadro colaborativo da Figma para ideias, dinâmicas e fluxos.', url: 'https://www.figma.com/pt-br/figjam/' },
  { title: 'Miro', description: 'Quadro colaborativo para dinâmicas, mapas e workshops.', url: 'https://miro.com/' },
  { title: 'Miroverse', description: 'Galeria de templates criados pela comunidade Miro.', url: 'https://miro.com/miroverse/' },
  { title: 'MURAL', description: 'Workspace visual para colaboração e facilitação.', url: 'https://www.mural.co/' },
  { title: 'Whimsical', description: 'Crie fluxogramas, wireframes e documentos visuais.', url: 'https://whimsical.com/' },
  { title: 'MindMeister', description: 'Mapas mentais online para organizar ideias.', url: 'https://www.mindmeister.com/' },
  { title: 'Milanote', description: 'Organize projetos criativos em quadros flexíveis.', url: 'https://milanote.com/' },
  { title: 'Witeboard', description: 'Quadro branco simples para colaboração em tempo real.', url: 'https://witeboard.com/' },
  { title: 'FlowMapp', description: 'Ferramenta para sitemaps, jornadas e fluxos de UX.', url: 'https://www.flowmapp.com/' },
]

export const guiaGravacoesDeTelaLinks: GuiaFerramentaLink[] = [
  ['OBS Studio','https://obsproject.com/','Gravação e transmissão de tela com recursos avançados.'],['Cisco Webex','https://www.webex.com/pt/index.html','Videoconferência e compartilhamento de tela.'],['Screenflick','https://www.araelium.com/screenflick-mac-screen-recorder','Gravador de tela para macOS com áudio e câmera.'],['Vidyard','https://chrome.google.com/webstore/detail/screen-and-webcam-recorde/jiihcciniecimeajcniapbngjjbonjan','Grave tela e webcam direto no navegador.'],['RecordScreen.io','https://recordscreen.io/','Grave a tela no navegador sem instalação.'],['Loom','https://www.useloom.com/','Mensagens em vídeo para trabalho e colaboração.'],['Overframe','https://overframe.xyz/','Grave protótipos com molduras de dispositivos.'],['Lookback','https://lookback.io/','Gravação de pesquisas com tela, voz e interações.'],['Awesome Screenshot','https://www.awesomescreenshot.com/','Captura de tela e gravação de vídeo no navegador.'],['Kap','https://getkap.co/','Gravador de tela open source para macOS.'],['Recordit','http://recordit.co/','Grave screencasts rápidos com suporte a GIF.'],['Inspectlet','https://www.inspectlet.com/','Análise de sessões e gravações de comportamento.'],['FullStory','https://www.fullstory.com/','Session replay e analytics de experiência digital.'],['Smartlook','https://www.smartlook.com/pt/','Gravações, mapas de calor e analytics de produto.'],['ScreenFlow','https://www.telestream.net/screenflow/overview.htm','Gravação e edição de vídeo para macOS.'],['Mobizen','https://play.google.com/store/apps/details?id=com.rsupport.mvagent&hl=pt_BR','Gravador de tela para Android.'],
].map(([title,url,description])=>({title,url,description}))
